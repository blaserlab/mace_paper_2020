...
all_cells_plot<-all_cells_plot+
  geom_point(size = 1, alpha = 0.2, shape = 21)+
...
overlay_plot<-function(foreground_data, background_data) {
  plot<-ggplot()+
    geom_point(data = background_data, aes(x = data_dim_1, y = data_dim_2), color = "grey80", fill = "grey80", shape = 21, alpha = 0.2)+
    geom_point(data = foreground_data, aes(x = data_dim_1, y = data_dim_2, color = cluster_assignment, fill = cluster_assignment), shape = 21, alpha = 0.2)+
    theme(legend.position = "none")+
    labs(x = "UMAP 1", y = "UMAP 2")
  return(plot)
